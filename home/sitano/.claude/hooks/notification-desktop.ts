...
function sendLinuxNotification(title: string, message: string): void {
  ...
  try {
    execSync(`notify-send --app-icon claude "${escapedTitle}" "${escapedMessage}"`, { stdio: 'ignore' })
  }
  catch {
    // Notification failed, ignore silently
  }
...
function detectPlatform(): 'mac' | 'linux' | 'unsupported' {
...
function main(): void {
  ...
  let notify: (title: string, message: string) => void
  ...
  if (platform === 'mac') {
    ...
    notify = sendLinuxNotification
  }
  else {
    console.warn('Claude Code Notification Hook: Unsupported platform, notifications will be disabled.')
    process.exit(1)
  }

  switch (notificationType) {
    case 'permission_prompt':
      notify('Claude Code - Permission Required', message || 'Claude needs your permission to continue')
      break
    case 'idle_prompt':
      notify('Claude Code - Waiting', message || 'Claude is waiting for your input')
      break
    default:
